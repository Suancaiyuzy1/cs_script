...
function Start_Ticks()
{
	...
	NPC_EFFECT_FIRE = Instance.FindEntityByName("temp_effect_01");
...
class class_npc_zombie
{
	...
	npc_items_data = new Map([
		["flame_detect_npc", {damage: 5}],
		["kar98_detect_npc", {damage: 100}],
		["shotgun_detect_npc", {damage: 80}],
		["misil_detect_npc", {damage: 1000}],
		["saw_detect_npc", {damage: 30}],
		["exp_barrel_detect_npc", {damage: 1000}]
	]);
	...
	ItemDamage(aData)
	{
		if(!aData.activator?.IsValid() || !aData.caller?.IsValid()) return;

		const item_name = aData.activator.GetEntityName();
		let result = item_name.substring(0, item_name.lastIndexOf("_"));

		let item = null;

		if(this.npc_items_data.has(item_name)) 
		{
			item = this.npc_items_data.get(item_name);
		}
		else if(this.npc_items_data.has(result)) 
		{
			item = this.npc_items_data.get(result);
		}

		if(item)
		{
			this.iHP_Base -= item.damage;
			const isFlame = (item_name === "flame_detect_npc" || result === "flame_detect_npc");

			if(isFlame)
			{
				const trigger = aData.caller;
				const currentTime = Instance.GetGameTime();
				let lastTime = this.triggerCooldowns.get(trigger) || 0;

				if(currentTime - lastTime >= 5.0)
				{
					this.triggerCooldowns.set(trigger, currentTime);

					let vec_body = this.lModel.GetAbsOrigin();
					const pos_f = {
						x: vec_body.x,
						y: vec_body.y,
						z: vec_body.z + 32
					...
					const spawn_ent = NPC_EFFECT_FIRE.ForceSpawn(pos_f);
					let particle = null;
					for(let i = 0; i < spawn_ent.length; i++)
					{
						let ent = spawn_ent[i];
					...
					if(particle?.IsValid())
					{
						particle.SetParent(this.lModel);
					}
				}
			}
	...
	BreakBase()
	{
		...
		const spawn_ent = NPC_EFFECT_HEAD_BROKE.ForceSpawn(pos_f);
		this.KillFade()
	}
...
function Input_Item_Damage_NPC(aData)
{
	let class_NPC = GetNPCClassByPhysBox(aData.caller);
	if (class_NPC == null)
	{
